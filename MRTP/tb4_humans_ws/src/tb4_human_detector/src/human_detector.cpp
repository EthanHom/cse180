...
// #include "navigation/navigation.hpp"   // Provides the Navigator class in MRTP
...
//   double cx() const { return sum_x / std::max(1, count); }
//   double cy() const { return sum_y / std::max(1, count); }
// };
...
//     navigator_(true)   // debug flag as in MRTP Listing 6.2
//   {
...
// //   const double cluster_dist_thresh_ = 0.6;   // meters
// //   const int min_points_per_cluster_ = 20;    // simple robustness threshold
//     const double cluster_dist_thresh_ = 1.2;   // meters, merge nearby points
//     const int min_points_per_cluster_ = 800;   // require substantial evidence
...
//     try {
//       tf = tf_buffer_.lookupTransform("map",
...
//     //   bool map_free = (occ >= 0 && occ < 50);    // threshold, adjust if needed
...
//       } else {
//         RCLCPP_WARN(node_->get_logger(), "Failed to reach waypoint.");
//       }
//     }
...
//    // Sequence to observe Human 1 at (1.04, -0.974)
...
//     // Get VERY close to Human 1
//     p.pose.position.x = 1.0;
//     p.pose.position.y = -1.0;
//     waypoints.push_back(p);

//     p.pose.position.x = 0.0;
//     p.pose.position.y = 2.0;   // Move north
//     waypoints.push_back(p);
    
//     // Move toward Human 2 at (-12, 15.2)
...
//     // CRITICAL: Get within 2-3m of Human 2
//     p.pose.position.x = -10.0;  // Just 1m away
//     p.pose.position.y = 16.0;   // About 1.5m away
//     waypoints.push_back(p);
    
//     // Optional: Circle around Human 2 for better coverage
//     p.pose.position.x = -13.0;
//     p.pose.position.y = 14.0;
//     waypoints.push_back(p);
    
//     p.pose.position.x = -12.0;
//     p.pose.position.y = 15.0;
//     waypoints.push_back(p);
...
//     if (good.empty()) {
//       RCLCPP_INFO(node_->get_logger(),
//                   "No significant environment changes detected; humans likely did not move.");
//       return;
//     }

//     RCLCPP_INFO(node_->get_logger(),
//                 "Detected %zu dynamic obstacle clusters (candidate humans):",
//                 good.size());
//     for (size_t i = 0; i < good.size(); ++i) {
//       RCLCPP_INFO(node_->get_logger(),
//                   "Human candidate %zu at (x=%.2f, y=%.2f) in map frame, cluster size %d",
//                   i, good[i].cx(), good[i].cy(), good[i].count);
//     }
//   }
//   */
...
//         // Filter: Only consider clusters in reasonable locations
//         // Exclude clusters too close to warehouse boundaries (likely walls)
//         if (std::abs(cx) > 13.0 || std::abs(cy) > 23.0) {
//             continue;  // Too close to outer walls
//         }
        
//         // Filter: Exclude horizontal/vertical wall alignments
//         // Many of your false positives are at y≈7.9, y≈17.0, y≈25.0 (horizontal corridors)
//         // This simple heuristic removes wall-aligned clusters
//         bool likely_wall = (std::abs(cy - 7.9) < 0.5) || 
//                             (std::abs(cy - 17.0) < 0.5) || 
//                             (std::abs(cy - 25.0) < 0.5) ||
//                             (std::abs(cx - 14.8) < 0.5) ||
//                             (std::abs(cx + 14.7) < 0.5);
...
//     // Sort by cluster size (descending) to prioritize large detections
...
//     // Report only top candidates (likely humans, not all obstacles)
//     size_t max_to_report = std::min(good.size(), size_t(5));  // Report top 5 at most
    
//     RCLCPP_INFO(node_->get_logger(),
//                 "Detected %zu high-confidence dynamic obstacles (top candidates for moved humans):",
//                 max_to_report);
//     for (size_t i = 0; i < max_to_report; ++i) {
//         RCLCPP_INFO(node_->get_logger(),
//                     "Candidate %zu at (x=%.2f, y=%.2f) in map frame, cluster size %d",
//                     i, good[i].cx(), good[i].cy(), good[i].count);
//     }
