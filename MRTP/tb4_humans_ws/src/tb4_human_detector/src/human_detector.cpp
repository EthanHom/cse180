...
// =============================================================================================
// MRTP FINAL PROJECT: OPPORTUNISTIC HUMAN DETECTOR
// Features: 
// 1. Heuristic Search (Check known spots first)
// 2. Active Interruption (Stops immediately if humans are detected en route)
// 3. Robust Wall Filtering (No false positives on corners)
// 4. High Speed (Terminates early)
// =============================================================================================
...
public:
          ...
          tf_listener_(tf_buffer_)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Opportunistic Human Detector Initialized.");
    }

    bool hasMap() const { return have_map_.load(); }
    
    // ---------------------------------------------------------
    // DETECTION STATUS CHECKERS
    // ---------------------------------------------------------
    
    bool isHuman1AtStart() const { 
        // Need decent evidence to say they are still there
        return (scans_near_h1_ > 10 && ((double)h1_hits_ / scans_near_h1_) > 0.1); 
    }
    
    bool isHuman2AtStart() const { 
        return (scans_near_h2_ > 10 && ((double)h2_hits_ / scans_near_h2_) > 0.1); 
    }

    // [NEW] Returns the number of valid "New" humans found in the map
    // This is called by Main to see if we can quit early.
    int getFoundNewHumanCount() {
        std::lock_guard<std::mutex> lock(data_mutex_);
        
        // 1. Filter raw hits
        std::vector<DetectionCluster> candidates;
        for (const auto& [coord, count] : dynamic_obstacles_) {
            // Must have >25 hits to be a solid object (not noise)
            if (count < 25) continue; 
            
            double wx = coord.first / 10.0; 
            double wy = coord.second / 10.0;

            // Ignore original spots (we handle those separately)
            if (std::hypot(wx - H1_X, wy - H1_Y) < 2.0) continue;
            if (std::hypot(wx - H2_X, wy - H2_Y) < 2.0) continue;

            // Boundary Check (Warehouse Floor only)
            if (wx < -13.5 || wx > 13.5 || wy < -23.5 || wy > 23.5) continue;

            candidates.push_back({wx, wy, count});
        }

        // 2. Cluster hits (merge points close to each other)
        std::vector<DetectionCluster> clusters;
        for (const auto& cand : candidates) {
            bool merged = false;
            for (auto& f : clusters) {
                if (std::hypot(f.x - cand.x, f.y - cand.y) < 1.5) {
                    merged = true;
                    break; 
                }
            }
            if (!merged) clusters.push_back(cand);
        }
        
        return clusters.size();
    }

    // [MRTP 9.1] Helper to check map value
    int8_t getMapValue(double x, double y) {
        std::lock_guard<std::mutex> lock(map_mutex_);
        if (!have_map_) return -1;
        int grid_x = static_cast<int>((x - map_.info.origin.position.x) / map_.info.resolution);
    ...
    std::vector<geometry_msgs::msg::PoseStamped> generateCoveragePath() {
        ...
        double step_size = 4.0; // 4m spacing is efficient
    ...
    void reportFindings() {
        ...
        bool h1_found = (scans_near_h1_ > 20 && ((double)h1_hits_ / scans_near_h1_) > 0.1);
        bool h2_found = (scans_near_h2_ > 20 && ((double)h2_hits_ / scans_near_h2_) > 0.1);

        std::vector<DetectionCluster> candidates;
        for (const auto& [coord, count] : dynamic_obstacles_) {
            if (count < 25) continue; // Noise filter
            
            double wx = coord.first / 10.0; 
            double wy = coord.second / 10.0;

            if (h1_found && std::hypot(wx - H1_X, wy - H1_Y) < 2.0) continue;
            if (h2_found && std::hypot(wx - H2_X, wy - H2_Y) < 2.0) continue;
...
private:
    ...
    void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr scan) {
        ...
        for (size_t i = 0; i < scan->ranges.size(); ++i) {
            float r = scan->ranges[i];
            if (!std::isfinite(r) || r < scan->range_min || r > 5.0) continue; // Limit range for confidence
            ...
            // ROBUST WALL FILTERING
            int map_val = get_map_val_unsafe(wx, wy);
            bool wall_nearby = false;
            int gx = static_cast<int>((wx - map_.info.origin.position.x) / map_.info.resolution);
            int gy = static_cast<int>((wy - map_.info.origin.position.y) / map_.info.resolution);
            
            // Check 20-cell radius (~60cm) to prevent false positives from walls
            int check_rad = 20; 
            for(int dy=-check_rad; dy<=check_rad && !wall_nearby; ++dy) {
                for(int dx=-check_rad; dx<=check_rad && !wall_nearby; ++dx) {
                    int idx = (gy + dy) * map_.info.width + (gx + dx);
                    if (idx >= 0 && idx < (int)map_.data.size() && map_.data[idx] == 100) wall_nearby = true;
                }
            }
...
int main(int argc, char **argv) {
    ...
    // Check if we accidentally found the moved humans already!
    int found_new = detector->getFoundNewHumanCount();
    
    std::cout << "[Main] Status: H1@" << (h1_found ? "Start" : "Moved") 
              << ", H2@" << (h2_found ? "Start" : "Moved") 
              << ". Found " << found_new << " new candidates en route." << std::endl;

    // ---------------------------------------------------------
    // PHASE 2: CONDITIONAL COVERAGE
    // ---------------------------------------------------------
    if (missing_count == 0) {
        std::cout << "[Main] SUCCESS: Both humans found at original locations!" << std::endl;
    } 
    else if (found_new >= missing_count) {
        // OPTIMIZATION: We found them while driving! Skip coverage!
        std::cout << "[Main] SUCCESS: Found moved humans en route! Skipping exhaustive search." << std::endl;
    }
    else {
        std::cout << "[Main] WARNING: Need to find " << (missing_count - found_new) << " more human(s). Starting Search..." << std::endl;
        
        auto waypoints = detector->generateCoveragePath();
        int wp_count = 0;
        for (const auto& wp : waypoints) {
            // EARLY EXIT CHECK
            // Before driving to next point, check if we found enough candidates
            if (detector->getFoundNewHumanCount() >= missing_count) {
                std::cout << "\n[Main] !!! FOUND ALL MISSING HUMANS !!! Stopping search early." << std::endl;
                break;
            }
            ...
            while (rclcpp::ok() && !navigator.IsTaskComplete()) { 
                // WHILE DRIVING check for humans too!
                if (detector->getFoundNewHumanCount() >= missing_count) {
                    navigator.CancelTask(); // Stop moving immediately
                    std::cout << " FOUND TARGET EN ROUTE!" << std::endl;
                    break;
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(100)); 
            }
            
            // Double check result in case we broke loop
            if (detector->getFoundNewHumanCount() >= missing_count) break;
